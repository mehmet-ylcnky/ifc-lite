...
- ifc-lite's shipped products are the **web viewer** + headless CLI/MCP/server. The desktop **capability** lives in `@ifc-lite/geometry` (`IPlatformBridge` / `NativeBridge` / `isTauri()`, `@tauri-apps/api` optional dep) — keep it web-pure (never imported on the web path; lazy-loaded only under `isTauri()`). A **reference desktop shell** exists at `apps/desktop` (Tauri v2) driving the `rust/desktop` engine crate (`ifc-lite-desktop-engine`, a thin wrapper over `ifc-lite-processing`). Both are deliberately **out of the monorepo build gates**: `pnpm-workspace.yaml` excludes `apps/desktop` (standalone, consumes the *published* `@ifc-lite/*`, own lockfile) and the root `Cargo.toml` excludes `apps/desktop/src-tauri` (needs Tauri/webkit2gtk sysdeps CI lacks). The engine `rust/desktop` **is** a workspace member and is unit-tested by `cargo test --workspace`, as is `rust/desktop-server` (`ifc-lite-desktop-server`) — a loopback WebSocket backend that streams the engine's packed shards to a plain browser tab (native speed without a desktop install; the `apps/desktop` frontend uses it when not in Tauri). The engine must not re-fork colour/coordinate/style resolution — it only adds the Z-up→Y-up boundary swap (mirroring `wasm-bindings/src/zero_copy.rs`) and the packed-shard wire encoder (byte-matching `packages/geometry/src/packed-geometry-decoder.ts`); the WS server and the Tauri shell are thin transports over it.
